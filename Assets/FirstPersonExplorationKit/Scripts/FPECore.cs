...
namespace Whilefun.FPEKit
{
    ...
    public class FPECore : MonoBehaviour
    {
        ...
        private void initialize()
        {
            ...
            // #Begin Plugin Change [KA]: Plugin mod so that I can predefine the spawned player and don't have to rely on this to spawn in a player.
            // GameObject player = Instantiate(playerPrefab, null);
            GameObject player = FindFirstObjectByType<FPEPlayer>().gameObject;

            if(player == null)
            {
                player = Instantiate(playerPrefab, null);
                Debug.LogWarning("WARNING:: FPECore::initialize --> player could not be found, instantiating player instead!");

            }
            // #End Plugin Change [KA]: Plugin mod so that I can predefine the spawned player and don't have to rely on this to spawn in a player.
